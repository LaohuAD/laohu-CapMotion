...
const validateDisplayName = (value) => {
	...
	if (name.length > 100)
		throw new Error("project display name must not exceed 100 characters");
	return name;
};

const syncProjectDisplayNameUnlocked = async ({
	projectPath,
	expectedRevision,
	prettyName,
	receiptPath,
	...
	const configTempPath = join(
		absoluteProject,
		`.project-config.${process.pid}.tmp`,
	);
	const metaTempPath = join(
		absoluteProject,
		`.recording-meta.${process.pid}.tmp`,
	);
	const metaRestorePath = join(
		absoluteProject,
		`.recording-meta.${process.pid}.restore.tmp`,
	);
	...
	try {
		...
		const requiredRevision = finiteRevision(
			expectedRevision,
			"expectedRevision",
		);
		...
		const nextName = validateDisplayName(
			prettyName ?? folderDisplayName(absoluteProject),
		);
...
export const syncProjectDisplayNameTransaction = async ({
	projectPath,
	expectedRevision,
	prettyName,
	receiptPath,
	...
	if (result.status !== 0)
		throw new Error(
			(
				result.stderr ||
				result.stdout ||
				"project name transaction failed"
			).trim(),
		);
...
if (isMain) {
	try {
		...
		if (!projectPath || expectedRevision === undefined) {
			throw new Error(
				"usage: cap-project-name.mjs --project PROJECT.cap --expected-revision N [--name DISPLAY_NAME] [--receipt receipt.json]",
			);
		}
		const options = {
			projectPath,
