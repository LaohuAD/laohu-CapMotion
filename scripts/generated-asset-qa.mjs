...
const frameRate = (value) => {
	const [numerator, denominator = "1"] = String(value ?? "0")
		.split("/")
		.map(Number);
	return denominator ? numerator / denominator : 0;
};

export function evaluateGeneratedAssetProbe(
	task,
	probe,
	...
	if (
		...
		Math.abs(duration - expectedDuration) > durationToleranceSeconds
	) {
		errors.push("ASSET_DURATION_MISMATCH");
	}
	if (
		task.type === "AVATAR" &&
		(width !== 1280 || height !== 720 || width <= height)
	) {
...
export async function verifyGeneratedAssets(
	frozenPackage,
	assets,
	{ readText = (path) => readFile(path, "utf8"), probe = probeMedia } = {},
) {
	...
	for (const task of expandAvatarTasks(frozenPackage.tasks)) {
		...
		if (!asset?.artifactPath)
			throw new Error(`GENERATED_ASSET_MISSING:${task.id}`);
		const qa = evaluateGeneratedAssetProbe(
			task,
			await probe(asset.artifactPath),
		);
		verified[task.id] = {
			...asset,
			width: qa.width,
...
async function main(argv) {
	...
	if (!frozenPath || !assetsPath || !outputPath) {
		throw new Error(
			"用法: generated-asset-qa.mjs <frozen.json> <assets.json> <verified-assets.json>",
		);
	}
	...
	const hardFailures = Object.entries(verified)
		.filter(([, asset]) => asset.qa?.hardStatus === "FAIL")
		.map(([id]) => id);
	process.stdout.write(
		`${JSON.stringify({ ok: hardFailures.length === 0, hardFailures, manualStatus: "OBSERVE" })}\n`,
	);
