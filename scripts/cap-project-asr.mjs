...
const FFPROBE_BIN =
	...
	(existsSync("/opt/homebrew/opt/ffmpeg@7/bin/ffprobe")
...
const sourceOffset = (segment, microphone) => {
	const starts = [
		...
		segment.camera,
		microphone,
		segment.system_audio ?? segment.systemAudio,
	]
		.map(startTime)
...
export async function buildCapAsrSourceIndex(
	projectInput,
	...
	if (Array.isArray(meta.segments)) {
		...
		throw new Error(
			`Unsupported or incomplete Cap recording metadata: ${metaPath}`,
		);
	}

	if (sources.length === 0)
		throw new Error("Cap project has no recording segments");
	...
	for (const source of sources) {
		if (!source.microphone) {
			throw new Error(
				`Recording segment ${source.recordingSegment} has no microphone track`,
			);
		}
		...
		await ensureFile(
			displayPath,
			`Recording segment ${source.recordingSegment} display`,
		);
		await ensureFile(
			microphonePath,
			`Recording segment ${source.recordingSegment} microphone`,
		);
		const displayDurationSeconds = Number(await probeDuration(displayPath));
		const microphoneDurationSeconds = Number(
			await probeDuration(microphonePath),
		);
		if (
			...
			throw new Error(
				`Invalid display duration for recording segment ${source.recordingSegment}`,
			);
		}
		if (
			!Number.isFinite(microphoneDurationSeconds) ||
			microphoneDurationSeconds <= 0
		) {
			throw new Error(
				`Invalid microphone duration for recording segment ${source.recordingSegment}`,
			);
		}
		const sourceTimelineOffsetSeconds =
			recordingType === "instant-embedded-audio"
				? 0
	...
	Array.isArray(words)
		? words.map((word) => {
				...
				return {
					...
					globalStartMs: Number.isFinite(sourceStartMs)
						? Math.round(baseMs + sourceStartMs)
						: null,
					globalEndMs: Number.isFinite(sourceEndMs)
						? Math.round(baseMs + sourceEndMs)
						: null,
				};
			})
		: [];
...
export function mergeSegmentTranscripts(sourceIndex, segmentResults) {
	...
	for (let index = 0; index < sourceIndex.segments.length; index += 1) {
		...
		const baseMs =
			(Number(source.globalOffsetSeconds) +
				Number(source.sourceTimelineOffsetSeconds)) *
			1000;
		...
		for (const utterance of sourceUtterances) {
			...
			utterances.push({
				...
				sourceStartMs,
				sourceEndMs,
				sourceTimelineStartMs: Math.round(
					source.sourceTimelineOffsetSeconds * 1000 + sourceStartMs,
				),
				sourceTimelineEndMs: Math.round(
					source.sourceTimelineOffsetSeconds * 1000 + sourceEndMs,
				),
	...
	if (utterances.length === 0)
		throw new Error("ASR results contain no timestamped utterances");
	...
	return new Promise((resolvePromise, reject) => {
		...
		child.on("close", (code) => {
			if (code === 0) resolvePromise({ stdout, stderr });
				...
				reject(
					new Error(
						`${command} exited with ${code}: ${(stderr || stdout).trim()}`,
					),
				);
		});
	});
...
const recognizeCapProject = async ({
	project,
	outputDir,
	planOnly = false,
	speaker = false,
	...
	if (planOnly) {
		return {
			...
			segmentCount: sourceIndex.segments.length,
		};
	}
		...
		for (const source of sourceIndex.segments) {
			...
			if (!DIRECT_FLASH_FORMATS.has(extension)) {
				...
				await runProcess(process.execPath, [
					join(ASR_SCRIPT_DIR, "extract-audio.mjs"),
					source.microphonePath,
					asrInput,
				]);
			}
			const args = [
			...
			segmentResults.push({
				rawJson,
				segmentSrt,
				payload: await readJson(rawJson),
			});
		}
	} finally {
	...
	await Promise.all([
		...
		writeFile(
			captionImportPath,
			`${JSON.stringify(merged.captionImport, null, 2)}\n`,
			"utf8",
		),
	]);
	const runRecord = {
...
const main = async () => {
	...
	if (!options.project || !options["output-dir"]) {
		console.error(
			"Usage: node scripts/cap-project-asr.mjs --project <recording.cap> --output-dir <directory> [--plan-only] [--speaker]",
		);
		process.exit(2);
	}
	try {
