...
const handoff = () => ({
	...
	edit: {
		...
		sourceToFinalMapPath: "/records/map.json",
	},
	tasks: [
		{
			id: "avatar-1",
...
test("exports avatar audio only from the frozen final main audio and records traceable hashes", async () => {
	const frozen = freezePostproductionPackage(handoff(), {
		edlContent: "edl",
		mappingContent: "map",
	});
	const commands = [];
	...
	const manifest = await prepareAvatarAudioJobs(frozen, {
		...
		readBytes: async (path) =>
			new TextEncoder().encode(
				path.includes("final-main") ? "source-audio" : `clip:${path}`,
			),
		makeDirectory: async () => {},
	...
	assert.deepEqual(
		manifest.jobs.map((job) => [job.start, job.end]),
		[
			[90, 120],
			[120, 145],
		],
	...
	assert(
		commands.every(
			({ args }) => args[args.indexOf("-i") + 1] === "/media/final-main.wav",
		),
	);
	assert(
		manifest.jobs.every(
			(job) => job.duration <= 40 && job.source === "FINAL_MAIN_AUDIO",
		),
	);
...
test("refuses a clipped avatar audio export whose actual duration does not match T2", async () => {
	...
	await assert.rejects(
		prepareAvatarAudioJobs(frozen, {
			...
			ffmpegBin: "ffmpeg",
			readText: async (path) =>
				path.endsWith("final.edl.json") ? "edl" : "map",
