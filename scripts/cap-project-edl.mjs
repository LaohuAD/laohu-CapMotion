...
const originalTimelineWindows = (segments) => {
  ...
  return segments.map((segment, index) => {
    ...
    const timescale = finite(segment.timescale ?? 1, `timeline.segments[${index}].timescale`);
    if (!(end > start) || !(timescale > 0)) throw new Error(`invalid source timeline segment ${index}`);
...
const resolveSequence = (config, edl) => {
  if (edl?.schema !== "laohu.cap-edl/1") throw new Error("unsupported EDL schema");
  if (!Array.isArray(edl.sequence) || edl.sequence.length === 0) throw new Error("EDL sequence is empty");
  const windows = originalTimelineWindows(config.timeline?.segments ?? []);
  let targetCursor = 0;

  return edl.sequence.map((entry, index) => {
    const sourceStart = finite(entry.sourceStart, `sequence[${index}].sourceStart`);
    const sourceEnd = finite(entry.sourceEnd, `sequence[${index}].sourceEnd`);
    const targetStart = finite(entry.targetStart, `sequence[${index}].targetStart`);
    const targetEnd = finite(entry.targetEnd, `sequence[${index}].targetEnd`);
    if (!nearlyEqual(targetStart, targetCursor)) throw new Error("target timeline must be contiguous");
    if (!(sourceEnd > sourceStart) || !(targetEnd > targetStart)) throw new Error(`invalid EDL range ${index}`);
    ...
    if (candidates.length !== 1) throw new Error(`EDL range ${index} does not resolve to exactly one source segment`);
    ...
    if (!nearlyEqual(targetEnd - targetStart, expectedDuration)) {
      throw new Error(`EDL range ${index} target duration does not match source duration`);
    }
    const originalTargetStart = sourceWindow.targetStart + (sourceStart - sourceWindow.sourceStart) / sourceWindow.timescale;
...
const remapKeys = (keys, originalSegment, clippedStart, clippedEnd) => {
  ...
  return keys.flatMap((key) => {
    ...
    if (eventTime < clippedStart - EPSILON || eventTime > clippedEnd + EPSILON) return [];
    return [{...key, timeOffset: Math.max(0, (eventTime - clippedStart) * 1000)}];
  });
};
...
const remapTrack = (items, sequence) => {
  ...
  for (const item of items ?? []) {
    ...
    for (const range of sequence) {
      ...
      const start = range.targetStart + (clippedStart - range.originalTargetStart);
      const end = range.targetStart + (clippedEnd - range.originalTargetStart);
      const next = {...item, start, end};
      if (piece > 0 && typeof next.id === "string") next.id = `${next.id}--edl-${piece + 1}`;
      if (Array.isArray(next.keys)) next.keys = remapKeys(next.keys, item, clippedStart, clippedEnd);
      mapped.push(next);
      piece += 1;
    }
  }
  return mapped.sort((left, right) => left.start - right.start || left.end - right.end);
};
...
const applyEdlUnlocked = async ({projectPath, edlPath, expectedRevision, receiptPath}) => {
  ...
  try {
    ...
    const currentRevision = finite(config.projectRevision ?? 0, "projectRevision");
    ...
    if (edl.sourceProjectRevision !== undefined && Number(edl.sourceProjectRevision) !== currentRevision) {
      throw new Error(`EDL source revision mismatch: expected ${currentRevision}, found ${edl.sourceProjectRevision}`);
    }
...
const applyCaptionTrackUnlocked = async ({projectPath, trackPath, expectedRevision, receiptPath}) => {
  const absoluteProject = resolve(projectPath);
  const configPath = join(absoluteProject, "project-config.json");
  const tempPath = join(absoluteProject, `.project-config.${process.pid}.tmp`);
  try {
    ...
    const inputTracks = isMultiTrack
      ? track.tracks
      : [{id: "default", label: "Captions", language: null, style: {}, segments: track.segments}];
    if (!Array.isArray(inputTracks) || inputTracks.length === 0) throw new Error("caption track input must contain tracks");
    ...
    const flattened = inputTracks.flatMap((inputTrack, trackIndex) => {
      ...
      if (!trackId || trackIds.has(trackId)) throw new Error(`invalid or duplicated caption track id ${trackId || trackIndex}`);
      trackIds.add(trackId);
      if (!Array.isArray(inputTrack.segments)) throw new Error(`caption track ${trackId} must contain a segments array`);
      ...
      return inputTrack.segments.map((segment) => ({
        ...
        ...(isMultiTrack ? {
          ...
          language: inputTrack.language == null ? null : String(inputTrack.language),
        ...
        ...(isMultiTrack ? {manualPositionOverride: segment.manualPositionOverride ?? style.manualPosition ?? null} : {}),
      }));
    });
    const segments = flattened.map((segment, index) => {
      const start = finite(segment.start, `caption track segment ${index} start`);
      const end = finite(segment.end, `caption track segment ${index} end`);
      if (!(end > start) || !String(segment.text ?? "").trim()) throw new Error(`invalid caption track segment ${index}`);
      if (segment.pairId) {
        const rangeKey = `${start.toFixed(6)}:${end.toFixed(6)}`;
        const previous = pairedRanges.get(segment.pairId);
        if (previous && previous !== rangeKey) throw new Error(`paired caption ${segment.pairId} must share one target range`);
      ...
      return {
        ...
        ...(isMultiTrack ? {manualPositionOverride: segment.manualPositionOverride ?? null} : {}),
      };
    }).sort((left, right) => {
      const trackOrder = [...trackIds];
      return trackOrder.indexOf(left.trackId) - trackOrder.indexOf(right.trackId)
    ...
    if (isMultiTrack) next.captions = {...(next.captions ?? {}), displayMode: "materialized"};
    next.projectRevision = currentRevision + 1;
    await writeFile(tempPath, `${JSON.stringify(next, null, 2)}\n`, "utf8");
    await rename(tempPath, configPath);
    const receipt = {
...
const runLockedWorker = ({workerFlag, projectPath, inputPath, expectedRevision, receiptPath}) => {
  const absoluteProject = resolve(projectPath);
  ...
  if (result.status !== 0) {
    throw new Error((result.stderr || result.stdout || "EDL transaction failed").trim());
  }
  return JSON.parse(result.stdout);
};

export const applyEdlTransaction = async ({projectPath, edlPath, expectedRevision, receiptPath}) => runLockedWorker({
...
export const applyCaptionTrackTransaction = async ({projectPath, trackPath, expectedRevision, receiptPath}) => runLockedWorker({
...
if (isMain) {
  try {
    ...
    const args = parseArgs(process.argv.slice(lockedWorker || captionTrackWorker ? 3 : 2));
    ...
    if (!projectPath || (!captionTrackMode && !edlPath) || (captionTrackMode && !trackPath) || expectedRevision === undefined) {
      throw new Error("usage: cap-project-edl.mjs --project PROJECT.cap (--edl final.edl.json | --track captions.json) --expected-revision N [--receipt receipt.json]");
    }
    const receipt = captionTrackMode
      ? await (captionTrackWorker ? applyCaptionTrackUnlocked : applyCaptionTrackTransaction)({projectPath, trackPath, expectedRevision, receiptPath: args.get("receipt")})
      : await (lockedWorker ? applyEdlUnlocked : applyEdlTransaction)({projectPath, edlPath, expectedRevision, receiptPath: args.get("receipt")});
