...
const overlayContinuity = (overrides = {}) => ({
	...
	protectedRegions: [
		{ target: "SUBTITLES", description: "底部双语字幕安全区" },
	],
...
const basePackage = () => ({
	...
	edit: {
		...
		preEditReview: {
			...
			userApproval: {
				status: "APPROVED",
				reference: "user-confirmed-review-turn",
...
test("rejects postproduction before the full pre-edit transcript review exists", () => {
	...
	assert(
		result.errors.some((error) => error.code === "PRE_EDIT_REVIEW_REQUIRED"),
	);
});
...
test("cut approval alone cannot authorize visual production", () => {
	...
	assert(
		result.errors.some(
			({ code }) => code === "PRE_EDIT_VISUAL_PLAN_INCOMPLETE",
		),
	);
	assert(
		result.errors.some(
			({ code }) => code === "PRE_EDIT_VISUAL_APPROVAL_REQUIRED",
		),
	);
	assert.throws(() =>
		freezePostproductionPackage(value, {
			edlContent: "edl",
			mappingContent: "map",
		}),
	);
...
test("unresolved visual feedback or materials blocks the whole production package", () => {
	for (const field of ["included", "feedbackResolved", "materialsResolved"]) {
		...
		assert(
			validatePostproductionPackage(value).errors.some(
				({ code }) => code === "PRE_EDIT_VISUAL_PLAN_INCOMPLETE",
			),
			field,
		);
...
test("feedback revising the reviewed plan requires approval of that revision", () => {
	...
	assert(
		validatePostproductionPackage(value).errors.some(
			({ code }) => code === "PRE_EDIT_APPROVAL_STALE",
		),
	);
...
test("legacy cut-only reviews remain valid without introducing visual work", () => {
	...
	value.edit.preEditReview.userApproval = {
		status: "APPROVED",
		reference: "cut-only-approval",
	};
	assert.equal(validatePostproductionPackage(value).ok, true);
	value.edit.preEditReview.visualPlan = {
		included: true,
		feedbackResolved: false,
		materialsResolved: true,
	};
	assert.equal(
		validatePostproductionPackage(value).ok,
		false,
		"requested visual planning still needs approval when the result selects only base footage",
	);
});
...
test("rejects postproduction while pre-edit questions or user approval remain unresolved", () => {
	...
	assert(
		result.errors.some(
			(error) => error.code === "PRE_EDIT_UNCERTAINTIES_UNRESOLVED",
		),
	);
	assert(
		result.errors.some(
			(error) => error.code === "PRE_EDIT_REVIEW_NOT_APPROVED",
		),
	);
});
...
test("rejects a pre-edit review that does not preview the final post-cut subtitle timeline", () => {
	...
	assert(
		result.errors.some(
			(error) => error.code === "PRE_EDIT_VIRTUAL_EDIT_INCOMPLETE",
		),
	);
});
...
test("rejects a pre-edit caption plan that inherits ASR cuts or splits atomic terms", () => {
	...
	assert(
		result.errors.some(
			(error) => error.code === "PRE_EDIT_CAPTION_REVIEW_INCOMPLETE",
		),
	);
});
...
test("rejects first-pass ownership and vague Remotion annotations", () => {
	...
	assert(
		result.errors.some(
			(error) => error.code === "REMOTION_ANNOTATION_INCOMPLETE",
		),
	);
});
...
test("requires every Remotion task to declare host-aware overlay continuity and contrast intent", () => {
	...
	assert(
		result.errors.some(
			(error) => error.code === "OVERLAY_CONTINUITY_INCOMPLETE",
		),
	);
});
...
test("accepts no scrim when the host already provides enough contrast", () => {
	...
	value.tasks[0].overlayContinuity.contrastReason =
		"深色真人背景与高亮白字已有足够对比，不重复压暗底画";
	const result = validatePostproductionPackage(value);
	assert.equal(result.ok, true, JSON.stringify(result.errors));
});
...
test("requires a real host compatibility contract when Remotion overlays AI video", () => {
	...
	assert.ok(
		missing.errors.some(
			(error) => error.code === "AI_HOST_COMPATIBILITY_INCOMPLETE",
		),
	);
...
test("rejects a persistent overlay group whose state breaks across carrier cuts", () => {
	...
	assert.equal(result.ok, false);
	assert(
		result.errors.some((error) => error.code === "OVERLAY_STATE_DISCONTINUITY"),
	);
});
...
test("rejects a technically complete package without audience intent or knowledge activation", () => {
	...
	assert(
		result.errors.some(
			(error) => error.code === "KNOWLEDGE_ACTIVATION_REQUIRED",
		),
	);
});
...
test("requires a complete KnowledgeVisualContract for UNDERSTAND Remotion and AI video tasks", () => {
	...
	assert(
		result.errors.some(
			(error) => error.code === "KNOWLEDGE_VISUAL_CONTRACT_INCOMPLETE",
		),
	);
	...
	aiVideo.tasks = [
		{
			...
			narrative: narrative(),
			knowledgeVisual: knowledgeVisual({
				segmentId: "ai-video-001",
				carrierDecision: "AI_VIDEO_FULL",
			}),
		},
	];
...
test("rejects an overlay task that has a time range but no viewer-state change", () => {
	...
	assert.equal(result.ok, false);
	assert(
		result.errors.some((error) => error.code === "TASK_NARRATIVE_INCOMPLETE"),
	);
});
...
test("rejects incomplete ownership and non-absolute handoff paths", () => {
	...
	assert.equal(result.ok, false);
	assert(
		result.errors.some((error) => error.code === "THREAD_HANDOFF_INVALID"),
	);
	assert(
		result.errors.some((error) => error.code === "SOURCE_PATH_NOT_ABSOLUTE"),
	);
	assert(
		result.errors.some((error) => error.code === "EDIT_PATH_NOT_ABSOLUTE"),
	);
});
...
test("requires a non-destructive Cap copy and separate linked bilingual caption tracks", () => {
	...
	assert.equal(result.ok, false);
	assert(
		result.errors.some((error) => error.code === "CAP_SOURCE_PROJECT_REQUIRED"),
	);
	assert(
		result.errors.some(
			(error) => error.code === "CAP_NON_DESTRUCTIVE_COPY_REQUIRED",
		),
	);
	assert(
		result.errors.some(
			(error) => error.code === "BILINGUAL_CAPTION_TRACKS_REQUIRED",
		),
	);
});
...
test("rejects an avatar segment over the hard 40 second limit", () => {
	...
	assert.equal(result.ok, false);
	assert(
		result.errors.some((error) => error.code === "AVATAR_SEGMENT_TOO_LONG"),
	);
});
...
test("expands a long avatar task only at declared semantic boundaries", () => {
	...
	assert.deepEqual(
		expanded
			.filter((task) => task.type === "AVATAR")
			.map((task) => [task.finalRange.start, task.finalRange.end]),
		[
			[90, 120],
			[120, 145],
		],
...
test("frozen package detects EDL or mapping changes before overlay work", async () => {
	...
	const okay = await verifyFrozenPackageFiles(frozen, {
		readText: async (path) =>
			path.endsWith("final.edl.json") ? "edl" : "mapping",
	});
	assert.equal(okay.ok, true);
	const stale = await verifyFrozenPackageFiles(frozen, {
...
test("overlay plan is revision chained and never rewrites the base timeline", () => {
	...
	const plan = buildOverlayCommandPlan(frozen, {
		"remotion-001": {
			definitionId: "timeline-proof",
			definitionVersion: 1,
			source: "motion/timeline-proof",
			compositionId: "TimelineProof",
		},
		"avatar-001": verifiedAsset(frozen, {
			artifactPath: "/media/avatar-001.mp4",
			width: 1280,
			height: 720,
			fps: 30,
		}),
	...
	assert.deepEqual(
		plan.map((step) => step.expectedRevision),
		[7, 8, 9],
	);
	assert.deepEqual(
		plan.map((step) => step.action),
		["motion-definition-register", "motion-add", "motion-artifact-import"],
	);
	assert.deepEqual(
		plan
			.filter((step) => step.action !== "motion-definition-register")
			.map((step) => step.role),
		["animation", "avatar"],
	);
	const overlays = plan.filter(
		(step) => step.action !== "motion-definition-register",
	);
	assert(
		overlays[0].zIndex > overlays[1].zIndex,
		"Remotion 动画应默认覆盖在数字人上方",
	);
...
test("long avatar tasks use the same semantic child IDs for audio generation and Cap overlays", () => {
	...
	const frozen = freezePostproductionPackage(value, {
		edlContent: "edl",
		mappingContent: "mapping",
	});
	const plan = buildOverlayCommandPlan(frozen, {
		"avatar-001-part-1": verifiedAsset(frozen, {
			artifactPath: "/media/avatar-part-1.mp4",
			width: 1280,
			height: 720,
			fps: 30,
		}),
		"avatar-001-part-2": verifiedAsset(frozen, {
			artifactPath: "/media/avatar-part-2.mp4",
			width: 1280,
			height: 720,
			fps: 30,
		}),
	...
	assert.deepEqual(
		plan.map((step) => step.segmentId),
		["avatar-001-part-1", "avatar-001-part-2"],
	);
	assert.deepEqual(
		plan.map((step) => [step.start, step.duration]),
		[
			[90, 30],
			[120, 25],
		],
	...
	assert.throws(
		() =>
			buildOverlayCommandPlan(frozen, {
				...
				"avatar-001": verifiedAsset(frozen, {
					artifactPath: "relative/avatar.mp4",
					width: 1280,
					height: 720,
					fps: 30,
				}),
...
test("screen-recording and evidence tasks become verified upper-track artifacts instead of being skipped", () => {
	...
	value.tasks = [
		{
			...
			finalRange: { start: 2, end: 6 },
			narrative: narrative({ purpose: "ACT" }),
		},
		{
			id: "evidence-001",
			type: "EVIDENCE",
			finalRange: { start: 8, end: 12 },
			narrative: narrative({ purpose: "TRUST" }),
		},
	];
	const frozen = freezePostproductionPackage(value, {
		edlContent: "edl",
	...
	assert.deepEqual(
		plan.map((step) => step.action),
		["motion-artifact-import", "motion-artifact-import"],
	);
	assert.deepEqual(
		plan.map((step) => step.segmentId),
		["screen-001", "evidence-001"],
	);
});
