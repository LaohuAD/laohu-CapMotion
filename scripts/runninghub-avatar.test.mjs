...
test("only sends final audio and locked workflow controls", () => {
	...
	assert.equal(
		nodes.some((node) => ["80", "169"].includes(node.nodeId)),
		false,
	);
});
...
test("batch runs each continuity group in order with at most five active groups", async () => {
	...
	const result = await runAvatarBatch(jobs, {
		...
		runJob: async (job) => {
			...
			active -= 1;
			if (job.id === "f-1")
				throw new Error("Bearer secret https://temporary/result");
...
test("run uses upload then safe node list then query and immediate download", async () => {
	...
	const fetchImpl = async (url, options = {}) => {
		...
		if (url.endsWith("/media/upload/binary"))
			return Response.json({ code: 0, data: { fileName: "server.wav" } });
		...
		if (url.endsWith("/query"))
			return Response.json({
				taskId: "task-1",
				status: "SUCCESS",
				results: [{ url: "https://download/result.mp4" }],
			});
		if (url === "https://download/result.mp4")
	...
	assert.equal(
		JSON.stringify(calls).includes("secret-value"),
		false,
		"test records must not retain the secret",
	);
});
	...
	await assert.rejects(
		runAvatarManifest(
			...
			},
			{
				...
				makeDirectory: async () => {},
				readBytes: async (path) =>
					bytes(path.endsWith("final-main.wav") ? "source" : "tampered"),
			},
		),
		...
		succeeded: [
			{
				id: "a",
				continuityGroup: "g1",
				result: { status: "SUCCESS", outputPath: "/tmp/a.mp4" },
			},
		],
		failed: [
			{ id: "b", continuityGroup: "g2", error: { code: "TASK_FAILED" } },
		],
		retryJobIds: ["b"],
		history: [
			{ attempt: 1, retriedJobIds: [], succeededIds: ["a"], failedIds: ["b"] },
		],
	};
				...
				jobs: [
					{
						id: "bad",
						source: "SCRIPT_TEXT",
						audioPath: "/tmp/bad.wav",
						audioSha256: "d".repeat(64),
					},
				],
			},
			{ outputDir: "/tmp/avatar-results", apiKey: "secret" },
		),
		/AVATAR_JOB_NOT_BOUND_TO_FINAL_AUDIO/,
	);
});
