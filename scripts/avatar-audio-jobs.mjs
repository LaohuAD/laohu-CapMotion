...
function runProcess(command, args) {
	return new Promise((accept, reject) => {
		...
		child.once("close", (code) =>
			code === 0
				? accept()
				: reject(new Error(`AUDIO_EXPORT_FAILED:${code}:${stderr.trim()}`)),
		);
	});
}
	...
	return new Promise((accept, reject) => {
		...
		child.once("close", (code) => {
			...
			if (code === 0 && Number.isFinite(duration) && duration > 0)
				...
				reject(new Error(`AVATAR_AUDIO_PROBE_FAILED:${code}:${stderr.trim()}`));
		});
	});
}

export async function prepareAvatarAudioJobs(
	frozenPackage,
	{
		outputDir,
	...
	if (!isAbsolute(outputDir ?? ""))
		throw new Error("AVATAR_AUDIO_OUTPUT_DIR_MUST_BE_ABSOLUTE");
	const sourceAudioPath = frozenPackage?.sources?.finalMainAudio;
	if (!isAbsolute(sourceAudioPath ?? ""))
		throw new Error("FINAL_MAIN_AUDIO_MUST_BE_ABSOLUTE");

	const frozenCheck = await verifyFrozenPackageFiles(frozenPackage, {
		readText,
	});
	if (!frozenCheck.ok)
	...
	const tasks = expandAvatarTasks(frozenPackage.tasks).filter(
		(task) => task.type === "AVATAR",
	);
	...
	for (const task of tasks) {
		...
		if (
			...
			Math.abs(actualDuration - duration) > 0.1
		) {
...
async function main(argv) {
	...
	if (!values.package || !values["output-dir"]) {
		throw new Error(
			"用法: avatar-audio-jobs.mjs --package <frozen.json> --output-dir </absolute/task-dir>",
		);
	}
	const frozenPackage = JSON.parse(
		await readFile(resolve(values.package), "utf8"),
	);
	const result = await prepareAvatarAudioJobs(frozenPackage, {
		outputDir: resolve(values["output-dir"]),
	});
	process.stdout.write(
		`${JSON.stringify({ ok: true, manifestPath: result.manifestPath, jobs: result.jobs.length })}\n`,
	);
}
...
if (resolve(process.argv[1] ?? "") === fileURLToPath(import.meta.url)) {
	main(process.argv.slice(2)).catch((failure) => {
		process.stderr.write(
			`${JSON.stringify({ ok: false, code: String(failure.message).split(":")[0] })}\n`,
		);
		process.exitCode = 1;
	});
}
