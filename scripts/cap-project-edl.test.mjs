...
test("remaps and clips source-timed editor tracks instead of leaving stale times", () => {
	...
	assert.deepEqual(
		next.timeline.zoomSegments.map(({ start, end }) => [start, end]),
		[
			[2, 5],
			[5, 6],
		],
	...
	assert.deepEqual(
		next.timeline.keyboardSegments.map(({ id, start, end }) => [
			id,
			start,
			end,
		]),
...
test("caption-track transaction materializes final-time captions without replacing the source master", async () => {
	...
	config.captions = {
		sourceTimed: true,
		segments: [{ id: "source-1", start: 2, end: 3, text: "源字幕" }],
	};
	await writeFile(configPath, `${JSON.stringify(config, null, 2)}\n`);
	await writeFile(join(projectPath, ".project-config.lock"), "");
	await writeFile(
		trackPath,
		`${JSON.stringify({ segments: [{ id: "source-1", start: 1, end: 2, text: "成片字幕" }] }, null, 2)}\n`,
	);

	try {
		const receipt = await applyCaptionTrackTransaction({
			projectPath,
			trackPath,
			expectedRevision: 7,
		});
...
test("caption-track transaction materializes linked Chinese and English as two editable tracks", async () => {
	const directory = await mkdtemp(
		join(tmpdir(), "cap-bilingual-caption-tracks-test-"),
	);
	const projectPath = join(directory, "test.cap");
	const configPath = join(projectPath, "project-config.json");
	...
	try {
		...
		assert.deepEqual(
			written.timeline.captionSegments.map((segment) => ({
				id: segment.id,
...
test("refit uses original source windows and tracks while retaining current presentation", () => {
	...
	current.captions = {
		settings: {
			fontWeight: 500,
			trackPositions: [{ trackId: "en", position: "manual" }],
		},
	};
	const again = applyEdlToProjectConfig(current, {
		...edl,
		sourceTimeline: original.timeline,
	});
	assert.deepEqual(
		again.timeline,
		applyEdlToProjectConfig(original, edl).timeline,
	);
