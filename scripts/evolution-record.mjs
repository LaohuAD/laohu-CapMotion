...
export function validateEvolutionRecord(record) {
	...
	if (!scopeValues.includes(record?.triggerScope))
		errors.push("TRIGGER_SCOPE_INVALID");
	if (!nonEmpty(record?.earliestFailure))
		errors.push("EARLIEST_FAILURE_REQUIRED");
	if (!layerValues.includes(record?.rootLayer))
		errors.push("ROOT_LAYER_INVALID");
	...
	if (!nonEmptyArray(record?.decisionChanges))
		errors.push("DECISION_CHANGE_REQUIRED");
	if (
	...
	for (const level of ["STRUCTURE", "ROUTE", "BEHAVIOR", "QUALITY"]) {
		if (!statusValues.includes(record?.validation?.[level]))
			errors.push(`VALIDATION_${level}_INVALID`);
	}
	if (!["KEEP", "OBSERVE", "REVERT"].includes(record?.decision))
