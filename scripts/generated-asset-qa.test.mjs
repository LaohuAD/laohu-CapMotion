...
test("screen recording and evidence overlays require 16:9 media and task-specific viewing checks", () => {
	for (const type of ["SCREEN_RECORDING", "EVIDENCE"]) {
		const result = evaluateGeneratedAssetProbe(
			{ ...avatarTask, type },
			{
				streams: [
					{
		...
		assert.equal(result.hardStatus, "PASS");
		assert(
			result.manualChecks.includes(
				type === "SCREEN_RECORDING" ? "operationState" : "claimSupport",
			),
		);
	}
	const invalid = evaluateGeneratedAssetProbe(
		{ ...avatarTask, type: "EVIDENCE" },
		{
			streams: [
				{
					codec_type: "video",
