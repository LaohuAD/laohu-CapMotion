...
export const WORKFLOW_EXPORT_SELECTS = Object.freeze({
	node241: "2",
	node244: "1",
	semanticStatus: "OBSERVE",
});
...
export function buildAvatarNodeInfoList(uploadedAudioFile) {
	...
	return [
		...
		{ nodeId: "233", fieldName: "value", fieldValue: "false" },
		{
			nodeId: "241",
			fieldName: "select",
			fieldValue: WORKFLOW_EXPORT_SELECTS.node241,
		},
		{
			nodeId: "244",
			fieldName: "select",
			fieldValue: WORKFLOW_EXPORT_SELECTS.node244,
		},
	];
}
...
export function splitAvatarSegments(segment, semanticBoundaries) {
	...
	if (segment.end - segment.start <= 40)
		return [{ ...segment, order: segment.order ?? 1 }];
...
export async function runAvatarBatch(jobs, { maxConcurrency = 5, runJob }) {
			...
			for (const job of group.jobs) {
				try {
					...
					failed.push({
						id: job.id,
						continuityGroup: group.id,
						error: sanitizeRunningHubFailure(failure),
					});
				}
			}
		}
	...
	await Promise.all(
		Array.from(
			{ length: Math.min(schedule.maxConcurrency, schedule.groups.length) },
			() => worker(),
		),
	);
...
export function sanitizeRunningHubFailure(failure) {
	...
	return {
		...
		responseMessage: String(
			failure?.response?.msg ?? failure?.response?.message ?? "",
		).replace(/https?:\/\/\S+/gi, "[EXPIRING_URL_REDACTED]"),
	};
}
...
export async function runAvatarJob({
	...
	const runResponse = await fetchImpl(RUNNINGHUB_ENDPOINTS.run, {
		...
		body: JSON.stringify({
			nodeInfoList: buildAvatarNodeInfoList(uploadedAudioFile),
		}),
	});
...
export async function readRunningHubApiKey(
	envPath = `${process.env.HOME}/.config/laohu/runninghub.env`,
) {
...
export async function runAvatarManifest(
	manifest,
	{
		outputDir,
		apiKey,
	...
	if (manifest?.schema !== "laohu.runninghub-avatar-audio-jobs/1")
		throw new Error("AVATAR_JOB_MANIFEST_INVALID");
	if (!isAbsolute(outputDir ?? ""))
		throw new Error("AVATAR_OUTPUT_DIR_MUST_BE_ABSOLUTE");
	const hashPattern = /^[a-f0-9]{64}$/;
	if (!isAbsolute(manifest.sourceAudioPath ?? ""))
		throw new Error("AVATAR_SOURCE_AUDIO_PATH_INVALID");
	if (
		![
			manifest.sourceAudioSha256,
			manifest.edlSha256,
	...
	if (
		digest(await readBytes(manifest.sourceAudioPath)) !==
		manifest.sourceAudioSha256
	) {
	...
	const jobs = retrySet
		? manifest.jobs.filter((job) => retrySet.has(job.id))
		: manifest.jobs;
	...
	const succeededById = new Map(
		(previousResult?.succeeded ?? []).map((item) => [item.id, item]),
	);
...
async function main(argv) {
	...
	if (!values.jobs || !values["output-dir"]) {
		throw new Error(
			"用法: runninghub-avatar.mjs --jobs <avatar-audio-jobs.json> --output-dir </absolute/task-dir> [--retry-from <runninghub-results.json>]",
		);
	}
	const apiKey = await readRunningHubApiKey();
	try {
		const manifest = JSON.parse(await readFile(resolve(values.jobs), "utf8"));
		const previous = values["retry-from"]
			? JSON.parse(await readFile(resolve(values["retry-from"]), "utf8"))
			: null;
		...
		process.stdout.write(
			`${JSON.stringify({ ok: true, resultPath: result.resultPath, succeeded: result.succeeded.length, failed: result.failed.length })}\n`,
		);
	} catch (failure) {
		process.stderr.write(
			`${JSON.stringify(sanitizeRunningHubFailure(failure))}\n`,
		);
		process.exitCode = 1;
	}
}
