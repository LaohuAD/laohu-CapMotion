...
export const buildCapEdlFromFrameEdl = ({
	...
	if (frameEdl?.schema !== "laohu.frame-edl/1")
		throw new Error("unsupported frame EDL schema");
	...
	for (const [rangeIndex, range] of frameEdl.sequence.entries()) {
		const globalStart = finite(
			range.sourceStart,
			`frame EDL range ${rangeIndex} sourceStart`,
		);
		const globalEnd = finite(
			range.sourceEnd,
			`frame EDL range ${rangeIndex} sourceEnd`,
		);
		const targetStart = finite(
			range.targetStart,
			`frame EDL range ${rangeIndex} targetStart`,
		);
		const targetEnd = finite(
			range.targetEnd,
			`frame EDL range ${rangeIndex} targetEnd`,
		);
		if (!nearlyEqual(targetStart, inputTargetCursor))
			throw new Error("frame EDL target timeline must be contiguous");
		if (
			!(globalEnd > globalStart) ||
			globalStart < -EPSILON ||
			globalEnd > totalSourceDuration + EPSILON
		) {
			throw new Error(
				`frame EDL range ${rangeIndex} is outside the Cap source timeline`,
			);
		}
		if (!nearlyEqual(globalEnd - globalStart, targetEnd - targetStart)) {
			throw new Error(
				`frame EDL range ${rangeIndex} source and target durations differ`,
			);
		}

		const pieces = windows.filter(
			(window) =>
				globalEnd > window.globalStart + EPSILON &&
				globalStart < window.globalEnd - EPSILON,
		);
		if (pieces.length === 0)
			throw new Error(
				`frame EDL range ${rangeIndex} did not resolve to source media`,
			);
		for (const window of pieces) {
			...
			const localStart =
				window.sourceStart +
				(pieceGlobalStart - window.globalStart) * window.timescale;
	...
	if (!nearlyEqual(outputTargetCursor, inputTargetCursor)) {
		throw new Error(
			"converted Cap EDL duration drifted from the approved frame EDL",
		);
	}

	return {
		schema: "laohu.cap-edl/1",
		sourceProjectRevision: finite(
			sourceProjectRevision,
			"sourceProjectRevision",
		),
...
export const buildCapBilingualTracks = ({ bilingual, durationSeconds }) => {
	...
	const normalized = bilingual.segments.map((segment, index) => {
		const start = finite(
			segment.targetStart ?? segment.start,
			`caption ${index} start`,
		);
		const end = finite(
			segment.targetEnd ?? segment.end,
			`caption ${index} end`,
		);
		...
		if (start < previousEnd - EPSILON)
			throw new Error(
				`bilingual caption ${index} overlaps the previous caption`,
			);
		if (end > duration + EPSILON)
			throw new Error(
				`bilingual caption ${index} extends past the edited timeline`,
			);
		previousEnd = end;
...
if (isMain) {
	try {
		...
		const tracks = buildCapBilingualTracks({
			bilingual,
			durationSeconds: capEdl.durationSeconds,
		});
		await Promise.all([
			writeFile(
				resolve(args.get("edl-output")),
				`${JSON.stringify(capEdl, null, 2)}\n`,
			),
			writeFile(
				resolve(args.get("tracks-output")),
