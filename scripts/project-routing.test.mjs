...
test("first-pass Cap is routed to the reusable ASR entry without creating a postproduction task", () => {
	assert.deepEqual(
		routeProjectTask({ phase: "FIRST_PASS", request: "CAP_TO_ASR" }),
		{
...
test("second-pass package is owned by one dedicated video task", () => {
	const route = routeProjectTask({
		phase: "SECOND_PASS",
		request: "POSTPRODUCTION_PACKAGE",
	});
...
test("all new program animation routes to Remotion", () => {
	assert.equal(
		routeProjectTask({ phase: "SECOND_PASS", request: "PROGRAM_ANIMATION" })
			.skill,
		"laohu-animation-director",
	);
});

test("specialized second-pass work reaches one accountable skill", () => {
	assert.equal(
		routeProjectTask({ phase: "SECOND_PASS", request: "AVATAR" }).skill,
		"runninghub-avatar",
	);
	assert.equal(
		routeProjectTask({ phase: "SECOND_PASS", request: "CORRECT_SRT" }).skill,
		"correct-srt-subtitles",
	);
	assert.equal(
		routeProjectTask({ phase: "SECOND_PASS", request: "BURN_SUBTITLES" }).skill,
		"burn-subtitles",
	);
});
...
test("project evolution has one owner and requires user authorization", () => {
	assert.throws(
		() => routeProjectEvolution({ authorized: false, request: "EVOLVE" }),
		/PROJECT_EVOLUTION_AUTHORIZATION_REQUIRED/,
	);
	assert.equal(
		routeProjectEvolution({ authorized: true, request: "EVOLVE" }).skill,
		"laohu-video-evolution",
	);
});
