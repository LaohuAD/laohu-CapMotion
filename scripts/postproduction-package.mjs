...
function knowledgeVisualContractIssues(
	contract,
	{ expectedCarrier, taskId } = {},
) {
	if (!contract || typeof contract !== "object")
		return ["KnowledgeVisualContract 缺失"];
	const issues = REQUIRED_KNOWLEDGE_VISUAL_FIELDS.filter(
		(field) => !nonEmpty(contract[field]),
	).map((field) => `缺少 ${field}`);
	if (!KNOWLEDGE_TYPES.includes(contract.knowledgeType))
		issues.push("knowledgeType 不受支持");
	if (
		!Array.isArray(contract.entities) ||
		contract.entities.length < 1 ||
		!contract.entities.every(nonEmpty)
	) {
		issues.push("entities 必须包含至少一个语义对象");
	}
	if (
		!Array.isArray(contract.stateChanges) ||
		contract.stateChanges.length < 1 ||
		!contract.stateChanges.every(nonEmpty)
	) {
	...
	if (
		...
		contract.semanticAnchors.some(
			(anchor) =>
				!nonEmpty(anchor?.spokenCue) || !nonEmpty(anchor?.visualEvent),
		)
	) {
	...
	if (taskId && contract.segmentId !== taskId)
		issues.push("segmentId 必须与任务 id 一致");
	return issues;
}

function overlayContinuityIssues(contract) {
	if (!contract || typeof contract !== "object")
		return ["overlay continuity contract 缺失"];
	const issues = REQUIRED_OVERLAY_CONTINUITY_FIELDS.filter(
		(field) => !nonEmpty(contract[field]),
	).map((field) => `缺少 ${field}`);
	if (!Number.isInteger(contract.order) || contract.order < 1)
		issues.push("order 必须为正整数");
	if (!OVERLAY_ROLES.includes(contract.role)) issues.push("role 不受支持");
	if (!HOST_CARRIERS.includes(contract.hostCarrier))
		issues.push("hostCarrier 不受支持");
	if (!OVERLAY_PERSISTENCE.includes(contract.persistence))
		issues.push("persistence 不受支持");
	if (!CONTRAST_MODES.includes(contract.contrastMode))
		issues.push("contrastMode 不受支持");
	if (
		!Array.isArray(contract.attentionPlan) ||
		contract.attentionPlan.length < 1 ||
		contract.attentionPlan.some(
			(item) =>
	...
	if (
		...
		contract.protectedRegions.some(
			(region) =>
				!PROTECTED_TARGETS.includes(region?.target) ||
				!nonEmpty(region?.description),
		)
	) {
...
function aiHostCompatibilityIssues(contract) {
	if (!contract || typeof contract !== "object")
		return ["AI host compatibility contract 缺失"];
	const issues = REQUIRED_AI_HOST_COMPATIBILITY_FIELDS.filter(
		(field) => !nonEmpty(contract[field]),
	).map((field) => `缺少 ${field}`);
	if (
		!Array.isArray(contract.forbiddenOverlayWindows) ||
		!contract.forbiddenOverlayWindows.every(nonEmpty)
	) {
...
export function validatePostproductionPackage(value) {
	...
	if (!value || typeof value !== "object") {
		return {
			ok: false,
			errors: [error("INVALID_PACKAGE", "$", "任务包必须是对象")],
		};
	}
	if (value.schema !== "laohu.video-postproduction-handoff/1") {
		errors.push(
			error(
				"UNSUPPORTED_SCHEMA",
				"schema",
				"只接受 laohu.video-postproduction-handoff/1",
			),
		);
	}
	...
	const missingIntent = REQUIRED_INTENT_FIELDS.filter(
		(field) => !nonEmpty(value.intent?.[field]),
	);
	if (missingIntent.length) {
		errors.push(
			error("INTENT_INCOMPLETE", "intent", `缺少 ${missingIntent.join(", ")}`),
		);
	}
	if (
		!Array.isArray(value.knowledgeActivations) ||
		value.knowledgeActivations.length < 1
	) {
		errors.push(
			error(
				"KNOWLEDGE_ACTIVATION_REQUIRED",
				"knowledgeActivations",
				"重要后期任务必须说明专业知识改变了哪次决定",
			),
		);
	} else {
		for (const [index, activation] of value.knowledgeActivations.entries()) {
			const missing = REQUIRED_ACTIVATION_FIELDS.filter(
				(field) => !nonEmpty(activation?.[field]),
			);
			if (missing.length) {
				errors.push(
					error(
						"KNOWLEDGE_ACTIVATION_INCOMPLETE",
						`knowledgeActivations[${index}]`,
						`缺少 ${missing.join(", ")}`,
					),
				);
			}
	...
	if (value.thread?.upstreamCreatesOnce !== true) {
		errors.push(
			error(
				"THREAD_HANDOFF_INVALID",
				"thread.upstreamCreatesOnce",
				"上游只创建并发送一次初始后期任务",
			),
		);
	}
	if (
		!nonEmpty(value.capProject?.path) ||
		!Number.isInteger(value.capProject?.expectedRevision) ||
		value.capProject.expectedRevision < 0
	) {
		errors.push(
			error(
				"CAP_PROJECT_INCOMPLETE",
				"capProject",
				"必须给出 Cap 路径和非负 expectedRevision",
			),
		);
	} else if (!isAbsolute(value.capProject.path)) {
		errors.push(
			error(
				"CAP_PROJECT_PATH_NOT_ABSOLUTE",
				"capProject.path",
				"Cap 工程路径必须是绝对路径",
			),
		);
	...
	if (
		!nonEmpty(value.capProject?.sourcePath) ||
		!isAbsolute(value.capProject?.sourcePath ?? "")
	) {
		errors.push(
			error(
				"CAP_SOURCE_PROJECT_REQUIRED",
				"capProject.sourcePath",
				"必须给出只读保留的原始 Cap 工程绝对路径",
			),
		);
	}
	if (
		value.capProject?.nonDestructiveCopy !== true ||
		value.capProject?.sourcePath === value.capProject?.path
	) {
		errors.push(
			error(
				"CAP_NON_DESTRUCTIVE_COPY_REQUIRED",
				"capProject",
				"后期必须写入独立的新 Cap 工程，不能修改原始录制工程",
			),
		);
	}
	if (
		!nonEmpty(value.sources?.secondPassAsr) ||
		!nonEmpty(value.sources?.productionScript) ||
		!nonEmpty(value.sources?.finalMainAudio)
	) {
		errors.push(
			error(
				"SOURCE_INPUT_INCOMPLETE",
				"sources",
				"必须给出第二遍 ASR、完整制作脚本和最终主音频",
			),
		);
	} else if (
		![
			value.sources.secondPassAsr,
			value.sources.productionScript,
			value.sources.finalMainAudio,
		].every(isAbsolute)
	) {
		errors.push(
			error(
	...
	if (
		!nonEmpty(value.edit?.edlPath) ||
		!nonEmpty(value.edit?.sourceToFinalMapPath)
	) {
		errors.push(
			error(
				"EDIT_MAPPING_INCOMPLETE",
				"edit",
				"必须给出最终 EDL 与 S2→T2 映射",
			),
		);
	} else if (
		![value.edit.edlPath, value.edit.sourceToFinalMapPath].every(isAbsolute)
	) {
		errors.push(
			error(
				"EDIT_PATH_NOT_ABSOLUTE",
				"edit",
				"EDL 与 S2→T2 映射必须使用绝对路径",
			),
		);
	}
	...
	if (
		...
		![captionTracks?.sourceMasterPath, captionTracks?.displayTracksPath].every(
			(path) => nonEmpty(path) && isAbsolute(path),
		) ||
	...
	if (!preEditReview || !nonEmpty(preEditReview.documentPath)) {
		errors.push(
			error(
				"PRE_EDIT_REVIEW_REQUIRED",
				"edit.preEditReview",
				"剪辑前必须提供覆盖整片的预剪辑字幕审稿文档",
			),
		);
	} else {
		if (!isAbsolute(preEditReview.documentPath)) {
			errors.push(
				error(
					"PRE_EDIT_REVIEW_PATH_NOT_ABSOLUTE",
					"edit.preEditReview.documentPath",
					"预剪辑字幕审稿文档必须使用绝对路径",
				),
			);
		}
		if (
			preEditReview.fullTimelineCovered !== true ||
			preEditReview.segmentBoundariesIncluded !== true
		) {
			errors.push(
				error(
					"PRE_EDIT_REVIEW_INCOMPLETE",
					"edit.preEditReview",
					"审稿文档必须覆盖整片并按每个候选剪辑片段列出源时码、字幕和处理意见",
				),
			);
		}
		if (
		...
		if (preEditReview.uncertaintiesResolved !== true) {
			errors.push(
				error(
					"PRE_EDIT_UNCERTAINTIES_UNRESOLVED",
					"edit.preEditReview.uncertaintiesResolved",
					"剪辑、字幕文字、专名、参数、字体或样式疑难未确认时不得进入剪辑",
				),
			);
		}
		if (
			preEditReview.userApproval?.status !== "APPROVED" ||
			!nonEmpty(preEditReview.userApproval?.reference)
		) {
			errors.push(
				error(
					"PRE_EDIT_REVIEW_NOT_APPROVED",
					"edit.preEditReview.userApproval",
					"必须记录用户对整份预剪辑字幕审稿文档的明确批准",
				),
			);
		}
		const requiresVisualReview =
			preEditReview.visualPlan !== undefined ||
		...
		if (requiresVisualReview) {
			if (
				preEditReview.visualPlan?.included !== true ||
				preEditReview.visualPlan?.feedbackResolved !== true ||
				preEditReview.visualPlan?.materialsResolved !== true
			) {
				errors.push(
					error(
						"PRE_EDIT_VISUAL_PLAN_INCOMPLETE",
						"edit.preEditReview.visualPlan",
						"同一审稿文档必须包含画面与动画方案，并解决反馈、素材来源或替代方案；未完成不得执行剪辑或覆盖制作",
					),
				);
			}
			if (preEditReview.userApproval?.scope !== "EDIT_AND_VISUALS") {
				errors.push(
					error(
						"PRE_EDIT_VISUAL_APPROVAL_REQUIRED",
						"edit.preEditReview.userApproval.scope",
						"剪口批准不能代替整份剪辑与画面方案批准",
					),
				);
			}
			if (
				!Number.isInteger(preEditReview.revision) ||
				preEditReview.revision < 1 ||
				preEditReview.userApproval?.reviewRevision !== preEditReview.revision
			) {
				errors.push(
					error(
						"PRE_EDIT_APPROVAL_STALE",
						"edit.preEditReview.userApproval.reviewRevision",
						"必须确认当前审稿修订；反馈改变时间、效果、材料或覆盖方式后不能沿用旧批准",
					),
				);
			}
		}
	...
	if (!Array.isArray(value.tasks))
		errors.push(error("TASKS_ARRAY_REQUIRED", "tasks", "tasks 必须是数组"));
	const ids = new Set();
	for (const [index, task] of (value.tasks ?? []).entries()) {
		const path = `tasks[${index}]`;
		if (!nonEmpty(task.id) || ids.has(task.id)) {
			errors.push(
				error("TASK_ID_INVALID", `${path}.id`, "任务 ID 必须非空且唯一"),
			);
		}
		ids.add(task.id);
		...
		const missingNarrative = REQUIRED_NARRATIVE_FIELDS.filter(
			(field) => !nonEmpty(task.narrative?.[field]),
		);
		if (missingNarrative.length) {
			errors.push(
				error(
					"TASK_NARRATIVE_INCOMPLETE",
					`${path}.narrative`,
					`缺少 ${missingNarrative.join(", ")}`,
				),
			);
		} else if (!NARRATIVE_PURPOSES.includes(task.narrative.purpose)) {
			errors.push(
				error(
					"TASK_PURPOSE_INVALID",
					`${path}.narrative.purpose`,
					`不支持 ${task.narrative.purpose}`,
				),
			);
		}
		if (
		...
		if (task.type === "REMOTION") {
			...
			if (missing.length) {
				errors.push(
					error(
						"REMOTION_ANNOTATION_INCOMPLETE",
						`${path}.annotation`,
						`缺少 ${missing.join(", ")}`,
					),
				);
			...
			if (task.overlayContinuity?.hostCarrier === "AI_VIDEO_FULL") {
				const compatibilityIssues = aiHostCompatibilityIssues(
					task.overlayContinuity.hostCompatibility,
				);
			...
			if (
				validRange(task.finalRange) &&
				task.finalRange.end - task.finalRange.start > 40 + 1e-6
			) {
				try {
					splitRangeAtSemanticBoundaries(
						task.finalRange,
						task.semanticBoundaries,
					);
				} catch {
					errors.push(
						error(
							"AVATAR_SEGMENT_TOO_LONG",
							`${path}.finalRange`,
							"超过 40 秒时必须提供能形成全部不超过 40 秒片段的自然语义边界",
						),
					);
				}
			}
			if (
				task.audio?.source !== "FINAL_MAIN_AUDIO" ||
				!nonEmpty(task.audio?.mapRef)
			) {
				errors.push(
					error(
			...
			if (
				!nonEmpty(task.continuity?.group) ||
				!Number.isInteger(task.continuity?.order) ||
				task.continuity.order < 1
			) {
				errors.push(
					error(
						"AVATAR_CONTINUITY_REQUIRED",
						`${path}.continuity`,
						"数字人任务必须提供连续组和顺序",
					),
				);
			...
			!["BASE", "SCREEN_RECORDING", "EVIDENCE", "AI_VIDEO"].includes(task.type)
		) {
			errors.push(
				error(
	...
	for (const entries of continuityGroups.values()) {
		entries.sort(
			(left, right) =>
				left.task.overlayContinuity.order - right.task.overlayContinuity.order,
		);
		for (let index = 1; index < entries.length; index += 1) {
			const previous = entries[index - 1];
			const current = entries[index];
			if (
				current.task.overlayContinuity.order !==
				previous.task.overlayContinuity.order + 1
			) {
				errors.push(
					error(
			...
			if (
				current.task.overlayContinuity.stateBefore !==
				previous.task.overlayContinuity.stateAfter
			) {
				errors.push(
					error(
...
function splitRangeAtSemanticBoundaries(
	range,
	semanticBoundaries = [],
	maxDuration = 40,
) {
	const candidates = [...new Set(semanticBoundaries)]
		.filter(
			(point) =>
				Number.isFinite(point) && point > range.start && point < range.end,
		)
	...
	while (range.end - start > maxDuration + 1e-6) {
		...
		const boundary = candidates
			.filter((point) => point > start && point <= ceiling)
			.at(-1);
...
export function expandAvatarTasks(tasks) {
	return tasks.flatMap((task) => {
		if (task.type !== "AVATAR" || !validRange(task.finalRange))
			return [structuredClone(task)];
		const ranges = splitRangeAtSemanticBoundaries(
			task.finalRange,
			task.semanticBoundaries,
		);
...
export function freezePostproductionPackage(
	value,
	{ edlContent, mappingContent },
) {
...
export async function verifyFrozenPackageFiles(
	value,
	{ readText = (path) => readFile(path, "utf8") } = {},
) {
	const errors = [];
	if (
		value?.state !== "EDL_FROZEN" ||
		value.freeze?.projectRevision !== value.capProject?.expectedRevision
	) {
	...
	if (sha256(mappingContent) !== value.freeze.mappingSha256)
		errors.push("MAPPING_STALE");
	return { ok: errors.length === 0, errors };
}
...
export function buildOverlayCommandPlan(value, assets) {
	...
	for (const task of expandAvatarTasks(value.tasks)) {
		...
		if (
			["AVATAR", "AI_VIDEO", "SCREEN_RECORDING", "EVIDENCE"].includes(
				task.type,
			) &&
		...
		if (task.type === "REMOTION") {
			...
			steps.push({
				...
				zIndex:
					asset.zIndex ??
					DEFAULT_Z_INDEX_BY_OVERLAY_ROLE[OVERLAY_ROLE_BY_TASK_TYPE[task.type]],
			});
		}
	}
	return steps;
...
export async function executeOverlayCommandPlan(
	plan,
	{ capBin = "cap", projectPath, runCommand = defaultRunCommand } = {},
) {
	if (!nonEmpty(projectPath)) throw new Error("CAP_PROJECT_PATH_REQUIRED");
	let revision = plan[0]?.expectedRevision ?? null;
	const outputs = [];
	for (const step of plan) {
		if (revision !== step.expectedRevision)
			throw new Error("OVERLAY_REVISION_CHAIN_BROKEN");
...
async function main(argv) {
	...
	if (
		!["validate", "freeze", "plan", "apply"].includes(command) ||
		!inputPath
	) {
		throw new Error(
			"用法: postproduction-package.mjs validate <package.json> | freeze <package.json> <output.json> | plan <frozen.json> <assets.json> | apply <frozen.json> <assets.json>",
		);
	}
	...
	if (command === "freeze") {
		...
		await writeFile(
			resolve(outputPath),
			`${JSON.stringify(freezePostproductionPackage(value, { edlContent, mappingContent }), null, 2)}\n`,
		);
		return;
	}
	...
	if (!frozenCheck.ok)
		throw new Error(`FROZEN_MAPPING_STALE:${frozenCheck.errors.join(",")}`);
	...
	process.stdout.write(
		`${JSON.stringify({ ok: true, revision: result.revision, steps: result.outputs.length })}\n`,
	);
}
...
if (resolve(process.argv[1] ?? "") === fileURLToPath(import.meta.url)) {
	main(process.argv.slice(2)).catch((failure) => {
		process.stderr.write(
			`${JSON.stringify({ ok: false, code: failure.message, errors: failure.errors ?? [] })}\n`,
		);
		process.exitCode = 1;
	});
}
