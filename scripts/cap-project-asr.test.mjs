...
const makeProject = async () => {
	...
	await mkdir(join(project, "content", "segments", "later-name"), {
		recursive: true,
	});
	await mkdir(join(project, "content", "segments", "earlier-name"), {
		recursive: true,
	});
	await writeFile(
		join(project, "content", "segments", "later-name", "mic.ogg"),
		"segment-zero",
	);
	await writeFile(
		join(project, "content", "segments", "earlier-name", "mic.ogg"),
		"segment-one",
	);
	await writeFile(
		join(project, "content", "segments", "later-name", "display.mp4"),
		"display-zero",
	);
	await writeFile(
		join(project, "content", "segments", "earlier-name", "display.mp4"),
		"display-one",
	);
	await writeFile(
		...
		JSON.stringify(
			{
				...
				segments: [
					{
						display: {
							path: "content/segments/later-name/display.mp4",
							fps: 30,
							start_time: 100.2,
						},
						mic: {
							path: "content/segments/later-name/mic.ogg",
							start_time: 100.0,
						},
					},
					{
						display: {
							path: "content/segments/earlier-name/display.mp4",
							fps: 30,
							start_time: 200.0,
						},
						mic: {
							path: "content/segments/earlier-name/mic.ogg",
							start_time: 200.1,
						},
					},
				],
...
test("builds the first-pass microphone index in recording order, not filename or edited order", async () => {
	...
	assert.deepEqual(
		index.segments.map((segment) => segment.recordingSegment),
		[0, 1],
	);
...
test("merges per-segment ASR with display-based offsets and preserves source timestamps", () => {
		...
		segments: [
			{
				...
				sourceTimelineOffsetSeconds: 0.2,
			},
			{
				recordingSegment: 1,
				globalOffsetSeconds: 5,
	...
	const segmentResults = [
		{
			...
			payload: {
				result: {
					utterances: [{ start_time: 100, end_time: 900, text: "第一段" }],
				},
			},
		},
		{
			...
			payload: {
				result: {
					utterances: [{ start_time: 250, end_time: 1250, text: "第二段" }],
				},
			},
		},
	];
	...
	assert.deepEqual(
		merged.raw.utterances.map((item) => item.sourceStartMs),
		[100, 250],
	);
	assert.deepEqual(
		merged.raw.utterances.map((item) => item.globalStartMs),
		[300, 5250],
	);
	...
	assert.deepEqual(
		merged.captionImport.segments.map((segment) => ({
			start: segment.start,
			end: segment.end,
			text: segment.text,
		})),
