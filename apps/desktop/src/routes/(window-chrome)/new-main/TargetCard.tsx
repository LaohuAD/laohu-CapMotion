...
export default function TargetCard(props: TargetCardProps) {
	...
	const subtitle = createMemo(() => {
		...
		if (recording) {
			return recording.mode === "studio"
				? t("capture.mode.studio")
				: t("capture.mode.instant");
		}
		return undefined;
	});
	...
	const handleCopy = async (e: MouseEvent) => {
		...
		try {
			...
			toast.success(t("capture.screenshotCopied"));
		} catch (error) {
			console.error("Failed to copy screenshot:", error);
			toast.error(t("capture.screenshotCopyFailed"));
		}
	};
	...
	const handleSave = async (e: MouseEvent) => {
		...
		try {
			...
			toast.success(t("capture.screenshotSaved"));
		} catch (error) {
			console.error("Failed to save screenshot:", error);
			toast.error(t("capture.screenshotSaveFailed"));
		}
	};
	...
	const handleShareScreenshot = async (e: MouseEvent) => {
		...
		try {
			...
			toast.success(t("capture.shareLinkCopied"), { id: toastId });
		} catch (error) {
			console.error("Failed to create screenshot share link:", error);
			const message = error instanceof Error ? error.message : String(error);
			toast.error(message || t("capture.shareLinkFailed"), { id: toastId });
		} finally {
	...
	const handleOpenRecordingFolder = (e: MouseEvent) => {
		...
		openRecordingFolder(recording.path, recording.mode).catch((error) => {
			...
			toast.error(t("capture.openFolderFailed"));
		});
	};
					...
					<div class="absolute inset-0 flex items-center justify-center bg-black/75">
						<div class="flex items-center gap-1 px-1.5 py-0.5 rounded-sm bg-red-9/20 text-red-11">
							...
							<span class="text-[10px] font-medium">
								{recordingFailed()
									? t("capture.recordingFailed")
									: t("capture.uploadFailed")}
							</span>
						</div>
					</div>
				</Show>
			...
			<div class="flex flex-col w-full">
				...
				<Show when={local.variant === "screenshot"}>
					<div class="flex items-center justify-between px-2 pb-1.5 pt-0.5 gap-1">
						<Tooltip content={t("capture.edit")}>
							<div
								role="button"
								tabIndex={-1}
						...
						<Tooltip content={t("capture.copyClipboard")}>
							<div
								role="button"
								tabIndex={-1}
								onClick={handleCopy}
						...
						return (
							<div class="flex items-center justify-between px-2 pb-1.5 pt-0.5 gap-1">
								...
								<Show when={recording.sharing}>
									<Tooltip content={t("capture.openLink")}>
										<div
											role="button"
											tabIndex={-1}
								...
								<Tooltip content={t("capture.openFolder")}>
									<div
										role="button"
										tabIndex={-1}
								...
								<Tooltip content={t("capture.delete")}>
									<div
										role="button"
										tabIndex={-1}
