...
export function PerformanceOverlay(_props: PerformanceOverlayProps) {
	...
	const copyStatsToClipboard = async () => {
		...
		toast.success(text("Performance stats copied to clipboard"));
	};
		...
		<Show when={performanceMode()}>
			...
			>
				<div
					...
					title={text("Click to copy stats")}
				>
					<div class="flex flex-col gap-0.5">
						...
						<Show when={stats().droppedFrames > 0}>
							<div style={{ color: "#f87171" }}>
								{text("Dropped:")} {stats().droppedFrames}/{stats().totalFrames}
							</div>
						</Show>
					</div>
				</div>
