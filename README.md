...
Agent 可以执行后台编译、自动测试和不会激活窗口的热更新。前端 HMR 可以更新已经打开但处于后台的 Cap 窗口，用户切换到其他软件后不得把 Cap 带回前台。Rust / Tauri 改动如果会导致应用进程重启并执行 `show`、`unminimize` 或 `set_focus`，默认只做后台编译和测试，不自动重启应用。未经用户明确要求“打开让我测试”，不得首次启动 Cap；用户允许测试后可以让应用保持打开，但后续更新仍不得抢占焦点。Agent 也不得自行启动已经运行的开发服务器。
