...
test("renders paired frame and audio-sample ranges without duration drift", async () => {
  ...
  execFileSync("ffmpeg", ["-y", "-v", "error", "-f", "lavfi", "-i", "color=size=192x108:rate=30:duration=1", "-f", "lavfi", "-i", "sine=frequency=440:sample_rate=48000:duration=1", "-c:v", "libx264", "-pix_fmt", "yuv420p", "-c:a", "aac", "-shortest", sourcePath]);
  ...
  const probe = JSON.parse(execFileSync("ffprobe", ["-v", "error", "-show_entries", "stream=codec_type,duration,nb_frames", "-show_entries", "format=duration", "-of", "json", outputPath], {encoding: "utf8"}));
