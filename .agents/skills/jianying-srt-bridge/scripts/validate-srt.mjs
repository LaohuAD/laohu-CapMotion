...
for (let blockIndex = 0; blockIndex < blocks.length; blockIndex += 1) {
  ...
  if (start === null || end === null) {
    errors.push(`Block ${blockIndex + 1}: invalid timestamp`);
    continue;
  }
  if (end <= start) errors.push(`Block ${blockIndex + 1}: non-positive duration`);
...
for (let index = 1; index < cues.length; index += 1) {
  ...
  if (current.start < previous.start) {
    errors.push(`Cue ${index + 1}: start time is earlier than the previous cue`);
  } else if (current.start < previous.end) {
    warnings.push(`Cue ${index + 1}: overlaps the previous cue by ${previous.end - current.start} ms`);
  }
}
...
if (mediaPath) {
  ...
  if (probe.status !== 0) {
    errors.push(`Unable to probe media: ${probe.stderr.trim() || "ffprobe failed"}`);
  } else {
    mediaDurationMs = Math.round(Number(probe.stdout.trim()) * 1000);
    const finalEnd = cues.at(-1)?.end ?? 0;
    if (finalEnd > mediaDurationMs + 1000) {
      errors.push(`Final cue exceeds media duration by ${finalEnd - mediaDurationMs} ms`);
    }
  }
}
